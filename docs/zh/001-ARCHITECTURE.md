...
amagi 不在本仓库内自研 OIDC protocol client。OIDC authorization-code / PKCE、callback exchange、refresh、userinfo、pending OAuth state 等基础设施采用 SecurityDept crates；Rust 侧推荐入口是 `securitydept-core`，打开 `token-set-context` 与 backend-oidc 所需 feature，并通过其 re-export 使用 `securitydept-token-set-context::backend_oidc_mode` 产品面。

Dashboard、extension popup、side panel、background sync API 共享的主认证基线应围绕 SecurityDept `token-set-context` 的 `backend-oidc` mode 组合。cookie session 后续可以作为 Dashboard 同域 UX 的可选补充，但不能成为浏览器插件认证或 sync API 的隐含前提。

auth 配置必须区分 OIDC client / token-set 与 OAuth resource server / access-token substrate，并支持 map-like 多 OIDC 来源。每个 source 应直接对齐 SecurityDept 的 `oidc`、`backend_oidc`、`frontend_oidc` 与 `access_token_substrate` typed config，而不是在 amagi 内再拆一层本地 `provider / oidc_client / *_override` 适配面。

amagi auth 模块仍然拥有应用侧职责：`/api/auth/token-set/oidc/source/{source}/start` facade route wiring、token-set state 接收与存储策略、extension/browser client session binding、OIDC account binding、auth user / domain user lookup、vault unlock session、domain authorization 与 audit event writing。SecurityDept token-set authenticated principal 不自动等于 vault access。

协议绑定的认证 endpoint 不使用 `/api/v1` 前缀。OIDC、token-set、WebAuthn / authenticator 等路径主要由 RFC / 安全流程约束；bookmark、library、sync profile、sync preview/apply 等业务资源接口继续使用 `/api/v1`。
...
- PostgreSQL
- OpenID Connect
  - 使用 [securitydept-core](https://github.com/ethaxon/securitydept) 作为 Rust 入口
  - OIDC / token-set 基线采用 SecurityDept `token-set-context` 的 `backend-oidc` mode
  - 不在 amagi route handler 内复制 authorization-code / PKCE / callback / refresh / userinfo flow
- WebAuthn
- OpenDAL
