...
- 本地 node id / tree 提取
- apply ops
- `browser.storage.local` 上的扩展 sync state 持久化
- platform capability detection
- 必要的 Chrome/Firefox/Safari compatibility override

当前 Iter9 baseline 已完成从过渡实现到共享包的迁移：

- `src/browser-bookmarks.ts`
- `src/browser-storage.ts`
- `src/capabilities.ts`
- fake `browser` 驱动的 Node 测试

不要再扩张成 Chromium / Firefox / Safari 三套长期维护的 adapter package；后续浏览器差异应继续收敛在 `packages/amagi-webext` 与 WXT target 配置中。
...
当前 Iter10 已把该阶段推进到以下 baseline：

- `packages/amagi-sync-client` 已提供 typed Sync API client、local tree normalization、diff baseline、apply plan baseline、manual sync orchestrator 和 Node tests。
- `packages/amagi-webext` 已提供 WebExtension bookmarks/storage adapter baseline、capability detection 和 fake-browser tests。
- `apps/extension-web` 已提供基于 WXT 的 background/popup/options 构建产物 baseline，并已验证 Chrome MV3 build + real extension load smoke、Firefox build + manifest smoke、Safari build + manifest smoke。
- server-created local node 的 mapping reconcile 已由 `amagi-sync-client` + `amagi-webext` 回填完成。

当前缺口：

- 尚未实现自动后台 sync、conflict resolution UI、完整 options/popup 状态管理。
- 已实现基于 `packages/amagi-auth-client` 的 extension token-set 登录闭环与 background manual sync bearer 注入；production self-hosted host 的 optional permission 授权仍留作后续项。
...
首阶段 MVP 先以"桌面浏览器 + Dashboard + 真实本地认证同步闭环"为核心。vault 基础能力仍是重要目标，但在可体验 happy-path demo 打通前不再作为当前阶段阻塞项。
...
- SecurityDept token-set 前端 SDK 接入
- 本地 Dex 登录、account binding、Dashboard/extension 手动同步闭环
...
- demo normal library
- sample sync profile
- demo vault library 在 vault 迭代开始时再提供；当前 happy-path demo 不应依赖 vault seed
...
### T10 收敛 WebExtension adapter、补齐 mapping reconcile，并建立 Chrome/Firefox/Safari smoke baseline
...
- `sync-core` 不直接依赖 WXT
- Chrome MV3 要有真实 extension load smoke；Firefox / Safari 保持 build + manifest smoke baseline
- `<all_urls>` 不进入当前 manifest；localhost host permission 与 dev-only bearer token 只作为开发入口

### T11 实现 sync profiles / rules UI 与 API

补充约束：

- `apps/dashboard-web` 在 Iter11 可以先实现单屏 sync management baseline，不要求一次到位成多页面 app
- dev bearer token + localStorage 只是开发入口，不是正式登录 UX
- 只有在真实出现多页面路由与共享异步状态需求时，再引入 TanStack Router / Query 等基础设施

### T12 打通本地 SecurityDept token-set 登录与手动同步 happy path

补充约束：

- `just dev` 应能启动 Postgres、Dex、API server、Dashboard Web 与 WXT extension，并提供一个可手动体验的本地闭环。
- Dashboard Web 与 extension 不再以手填 `devBearerToken` 作为主路径；主路径必须来自 SecurityDept token-set 前端 SDK 的 auth snapshot / authorization header。
- 本地 Dex 使用 `amagi/amagi`，OIDC source 使用 `default`，client id / secret 与提交的 local demo config 对齐。
- demo 闭环至少覆盖：登录、account binding、创建或读取 normal library、extension register/start session、preview、apply、ack 或等价的 cursor 推进验证。
- dev-only bearer token 可以临时保留为测试 fallback，但 UI 和文档必须明确它不是默认 happy path。

### T13 实现 vault library + unlock session + WebAuthn
