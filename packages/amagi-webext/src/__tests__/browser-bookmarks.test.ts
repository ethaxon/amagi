...
import { createWebExtBookmarkAdapter } from "../index";

describe("createWebExtBookmarkAdapter", () => {
	it("loads bookmark tree without the browser root wrapper", async () => {
		const adapter = createWebExtBookmarkAdapter({
			browser: {
				bookmarks: {
					async getTree() {
						return [
	...
	it("applies create update move delete ops through bookmark APIs", async () => {
		...
		const applyResult = await adapter.applyLocalPlan([
			{
				kind: "create",
				phase: 1,
				serverNodeId: "server-created-1",
		...
		expect(applyResult.createdMappings).toEqual([
			{
				serverNodeId: "server-created-1",
				clientExternalId: "created-1",
			},
		]);
	});

	it("prefers removeTree for folder delete when both remove APIs exist", async () => {
		const calls: string[] = [];
		const adapter = createWebExtBookmarkAdapter({
			browser: {
				bookmarks: {
		...
		await adapter.applyLocalPlan([
			{
				kind: "delete",
				phase: 4,
				clientExternalId: "folder-1",
			},
		] satisfies LocalApplyOp[]);

		expect(calls).toEqual(["removeTree:folder-1"]);
	});
});
